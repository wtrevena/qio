...
| File | Purpose |
|---|---|
| `main.tex` | Complete manuscript (round-3 revision, June 2026: retitled; rotor section moved to an appendix as a dynamical control; Jordan–Wigner gauge-covariance obstruction placed before the shortened RG section; Haar genericity compressed to a sanity check; notation table added; Latin Modern/T1 fonts). The bibliography is embedded as a `thebibliography` environment, so no `.bbl` or `.bib` file is needed. |
| `fig1_experiments.pdf` | Figure 1 (Section 3; panel (a) also referenced from Section 6). Figure text converted to outlines — no Type 3 fonts. |
| `fig2_entropy_flow.pdf` | Figure 2 (Section 6 entropy-flow consistency envelope). Figure text converted to outlines — no Type 3 fonts. |

No custom style files are used (standard `article` class with geometry, fontenc/lmodern, amsmath, amssymb, graphicx, hyperref — all in every TeX Live, including arXiv's). arXiv will run pdflatex automatically (AutoTeX); the figures are PDF, which pdflatex handles natively. `pdffonts` on the compiled PDF shows all fonts Type 1 and embedded (zero Type 3).
...
Motivated by Szangolies's three-qubit octonionic Hopf construction of the SU(3)×SU(2)×U(1)/Z₆ gauge group, we test a natural but naive extension of it: that the measured three-gauge-coupling hierarchy could be encoded, under a two-parameter logarithmic entropy map, in the single-qubit entanglement entropies of a three-qubit candidate state. We are not refuting Szangolies's construction; we are refuting this coupling-entropy extension, with scoped no-go results verified by published code. First, underconstraint: the map collapses to one entropy-gap-ratio condition lying strictly inside the image of the three-qubit marginal polytope, so the matching set is positive-dimensional and matching carries no evidence; a sampling check across 10⁷ Haar-random states confirms the matching manifold is statistically generic, and null controls (random coupling targets, four-qubit systems, relabeled qubits) confirm that parameter freedom drives matching. We exhibit an explicit analytic one-parameter family of W-class (zero 3-tangle) states satisfying the ratio condition exactly, so matching does not even fix the SLOCC class. Principal obstruction: recognizing Furey's Cl(6) as the three-qubit operator algebra (Jordan–Wigner) yields charge as Hamming-weight grading — and shows that gauge transformations are not local unitaries of the qubit factorization. The naive entropy observable is not gauge-invariant, so the map is not physically meaningful as stated. Independently, a scale-independent entropy triple is inconsistent with renormalization-group running; the surviving affine entropy-flow map is only a consistency envelope. An appendix supplies a dynamical control: the algebra-canonical rotor Hamiltonian i(L_u + 2R_u) selects a unique, convention-invariant, permutation-symmetric ground state (√7 e₀ − iu)/√14: even the most canonical dynamical octonionic construction produces symmetry, not hierarchy.
...
- Comments field suggestion: "21 pages, 2 figures, 2 tables. Code and raw outputs at [repository URL]." — fill in the actual code repository URL before submitting, since the manuscript states the code is published.
