...
Motivated by two convergent research programs — emergent spacetime from entanglement (Jacobson, Ryu-Takayanagi, holographic quantum error correction) and Standard Model structure from the octonions (Günaydin-Gürsey, Dixon, Furey, Todorov), recently bridged by Szangolies's identification of SU(3) × SU(2) × U(1)/Z₆ as the residual symmetry of a three-qubit octonionic Hopf fibration — we pose a sharp question: is the Standard Model gauge coupling hierarchy encoded in the entanglement entropies of a three-qubit vacuum state? We answer with a systematic set of no-go results, all verified computationally with published code. Statically, the question is underconstrained: the observed hierarchy at M_Z lies inside the image of the one-qubit marginal polytope, and across 10⁷ Haar-random states the coupling-matching manifold is statistically generic in the 3-tangle, intersects the W-class (zero 3-tangle) sector, and is matched equally well by random coupling targets, by four-qubit systems, and under all qubit-gauge relabelings. Two-qubit states cannot encode unequal couplings at all (Schmidt). A scale-independent entropy triple is inconsistent with renormalization-group running, whose gap ratio varies by ~24% between M_Z and 10⁶ GeV and is singular near 10¹³ GeV. Every canonical octonionic state construction tested — multiplication-table sign maps, Fano incidence, quaternionic lines, preferred complex directions — yields permutation-degenerate entropies incapable of any hierarchy. An exactly solvable case sharpens this rigidity: the rotor Hamiltonian H = Σ_{a<b} σ(a,b)·iL_aL_b built from the octonion multiplication-table signs equals i(L_u + 2R_u) for u the sum of the imaginary units, and its unique ground state (√7 e₀ − iu)/√14 — provably convention-invariant, and confirmed so across all 128 sign gauges and all 5040 index relabelings — is permutation-symmetric with exact invariants: single-qubit marginal spectra (1/7, 6/7), and 3-tangle equal to each squared pairwise concurrence, 8/49. Finally, making explicit that Furey's Cl(6) is the three-qubit operator algebra (three fermionic modes, via Jordan-Wigner) shows that electric charge is Hamming-weight grading and the color-symmetric quark combination is the W state — but also that gauge transformations are not local unitaries of the qubit factorization, so single-qubit entanglement entropies are gauge-frame dependent; the GUT normalization of the abelian coupling (α₁ = (5/3)α_Y) further renders the target hierarchy itself convention-dependent. The title question is therefore ill-posed as it stands, and we map precisely what survives: a coupling-entropy correspondence must be built from gauge-invariant entanglement functionals; it must generate the hierarchy by flow rather than statics — we construct the unique RG-consistent affine form α_i⁻¹(μ) = A + B·S_i(μ), whose quantum-marginal feasibility requires |B| ≳ 126 and whose single falsifiable consequence is that an entanglement-symmetric boundary state requires exact coupling unification (the Standard Model's entanglement asymmetry never reaches zero, bottoming at ≈ 0.029 near 2.4 × 10¹⁴ GeV; the MSSM reaches ≈ 0.003 at 1.4 × 10¹⁶ GeV) — and it must explain why the algebra's own canonical vacuum is symmetric. We present this as a map of the dead and surviving regions of a hypothesis space, not as evidence for the framework that motivated it (discussed as the "Quantum Information Ontology," an organizing heuristic only). We do not claim to derive the Standard Model, resolve quantum gravity, or solve the cosmological constant problem.
...
These programs jointly motivate a specific quantitative question, which is the subject of this paper: *is the Standard Model gauge coupling hierarchy encoded in the entanglement entropies of a three-qubit vacuum state?* The motivating framework — which we name the Quantum Information Ontology (QIO) and treat strictly as an organizing heuristic (Section 4) — would suggest yes. The body of the paper answers no, in every form of the question we can make precise: we prove the static version is underconstrained, execute the computational program testing it (Section 6), and find every result on the negative-diagnostic side. We then map what survives. Three objects bound the surviving hypothesis space: an entropy-flow form of the conjecture, the unique form consistent with renormalization-group running (Section 7); an exactly solvable octonionic vacuum-selection toy whose canonical vacuum is provably convention-invariant and symmetric (Section 8); and a Jordan–Wigner dictionary making the Furey construction's three-qubit content explicit, whose principal yield is that the program's central quantity — entanglement of the qubit factors — is not gauge-invariant (Section 9). The honest conclusion is that the title question is ill-posed until a gauge-invariant entanglement functional is specified, and we delineate exactly what such a specification must provide.
