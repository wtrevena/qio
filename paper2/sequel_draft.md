...
(verified on all five stored curve points; max |S_𝒜| = 1.9 × 10⁻¹⁵ — and exact by the support argument, since the conclusion depends only on the curve lying inside the weight-1 sector). The entire matching curve is one U(3) gauge orbit point: gauge-equivalent to a single d̄ quark in a rotated color frame. The gauge equivalence, too, holds for the whole curve by a one-line argument, not by sampling: SU(3) acts transitively on the unit sphere of ℂ³, so any unit weight-1 amplitude vector (a, b, c) is the third column of some U ∈ SU(3), and the corresponding state is G(U)|001⟩ exactly — the five stored points are numerical confirmation, not the proof. *The static coupling-matching analysis of [1], Secs. 5–6, was matching gauge artifacts.* The companion paper diagnosed its title question as ill-posed; the well-posed functional goes further and evaluates the would-be evidence at exactly zero.
...
[18] Particle Data Group, "Review of Particle Physics," Prog. Theor. Exp. Phys. 2022, 083C01 (2022) — coupling values entering r_SM = 1.8174 via [1], Eqs. (2)–(3). (Cited in Appendix B.3.)
...
* `strengthen.py` — the machine-checked lemmas of Appendix B (the mode-gauge-group lemma with its Bogoliubov, particle-hole, and antilinear variants; the SU(3)-centralizer computation; the hypercharge-normalization rerun of the assignment zoo; and the cross-paper weighted-W point reconciliation of Section 5.2); fixed seed 20260609; writes `strengthen_results.json`.

Run: `python run_direction_A.py` (≈ 6 s; NumPy + SciPy only), then `python strengthen.py` for Appendix B. The scripts read `../experiments/results/exp2_results.json` (published with [1]) for the (five) stored weighted-W matching-curve amplitudes tested in Section 5.4.
...
## Appendix B. Machine-Checked Lemmas
...
### B.1. The Mode-Preserving Unitary Group Is {e^{iφ} G(U) : U ∈ U(3)}

**Lemma B.1.** Let F = span_ℂ{f₁, f₂, f₃} ⊂ M₈(ℂ) be the span of the three Jordan–Wigner modes. Then:
...
### B.2. The Commutant Identifications Are Exact (Sandwich Argument)

**Lemma B.2.** For each gauge choice 𝒢 ∈ {U(3), SU(3), U(1)}, the commutant {G(g) : g ∈ 𝒢}′ is exactly the algebra identified in Section 4: the abelian ℂ⁴ of weight projectors (dimension 4), M₂ ⊕ ℂ ⊕ ℂ with the M₂ block on span{|000⟩, |111⟩} (dimension 6), and M₁ ⊕ M₃ ⊕ M₃ ⊕ M₁ of all weight-preserving operators (dimension 20), respectively.
...
For U(3) the identification also follows from Schur's lemma alone: the four weight sectors carry the inequivalent irreps 1, U, Λ²U, det U, each with multiplicity one, so the commutant is the span of the four sector projectors.

### B.3. The Zoo Miss Is Normalization-Independent

**Lemma B.3 (computational).** With the abelian coupling in hypercharge normalization, α_Y⁻¹ = (5/3)α₁⁻¹ = 98.33, the target gap ratio becomes r_Y = 1.0433 in place of the GUT-normalized r_SM = 1.8174 — and the identical 60-assignment sector-to-coupling zoo of Section 6.4, run on the rotor vacuum's exact invariant data p = (1/2, 3/14, 3/14, 1/14) against r_Y with the same 0.05 window, again finds nothing: no assignment within 0.05, none within 0.01. The conclusion of Section 6.4 is normalization-independent.
