...
\title{Gauge-Invariant Algebraic Entropy in a Minimal Fermionic Toy:\\ The Coupling--Entropy Question Made Well-Posed --- and Closed}
...
Numerical confirmation: all five stored curve points give $\max |\SA| = 1.9 \times 10^{-15}$, and the explicit $W = G(U)|001\rangle$ construction of Section~5.3 has residual $0.0$. The entire matching curve is gauge-equivalent to a single $\bar d$ quark in a rotated color frame, with invariant data identical to that of $|001\rangle$. \emph{The static coupling-matching analysis of \cite{companion}, Secs.~5--6, was matching gauge artifacts.} The companion paper diagnosed its title question as ill-posed; the well-posed functional goes further and evaluates the would-be evidence at exactly zero.
...
\textbf{Status: machine-checked, deliberately scoped lemma (Appendix~C.1) plus structural identification from Furey's construction.}
...
\section{The Two-Ideal Extension: A Concrete Proposal}
