...
That paper closed by identifying the construction of a gauge-invariant entanglement functional as the central open problem (\cite{companion}, Secs.~6.3 and~9). This paper completes that program for the minimal toy, and reports what the completed version says.
...
\textbf{Status: construction inherited from \cite{companion}, Sec.~6; verified numerically here to stated residuals.}
...
\textbf{The rotor vacuum} --- the unique ground state of the algebra-canonical Hamiltonian $H = i(L_u + 2R_u)$ of \cite{companion}, Sec.~7 --- is the only state in the list with nontrivial invariant data, and that data is two numbers (Section~6.3). Its invariant entropy, 1.724 bits, is comfortably \emph{generic}: the Haar mean is $1.580 \pm 0.239$. Gauge-invariantly, the canonical vacuum is not even unusually entangled.
...
\bibitem{twoideal} W.T. Trevena, ``[title to be finalized],'' companion paper, in preparation (2026); code and results report in the repository directory \texttt{newwork/two\_ideal/}.
