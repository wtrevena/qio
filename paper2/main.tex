...
\title{Gauge-Invariant Entanglement in a Minimal Fermionic Toy:\\ The Coupling--Entropy Question Made Well-Posed --- and Closed}
...
\textbf{Status: construction inherited from \cite{companion}, Sec.~9; machine-verified here to stated residuals.}
...
\end{equation*}
the \textbf{algebraic entanglement entropy} is
\begin{equation}
...
\textbf{Pitfall (machine-cross-checked on every call).} $\SA$ is \emph{not} the von Neumann entropy of the $d \times d$ matrix $\EA(\rho)$; the two differ by $\sum_k p_k \log_2 m_k$, because $\EA(\rho)$ spreads each block uniformly over its multiplicity factor. The library computes $\SA$ both ways and asserts agreement to $10^{-8}$.
...
\section{Gauge-Invariant Entanglement of the Named States}

\textbf{Status: computed; closed forms machine-checked to $10^{-12}$.}

\subsection{The Quantum Piece Vanishes Identically}

\textbf{Proposition 1.} \emph{For every pure state $|\psi\rangle \in \CC^8$ and each of the three algebras above, the quantum piece of Eq.~(\ref{eq:salg}) vanishes: $\SA(\psi) = H(\{p_k\})$.}
...
In Casini--Huerta language: for pure states, this toy is \emph{all edge-mode term, no bulk term}. Any claim that ``vacuum entanglement encodes couplings'' in this framework is therefore, gauge-invariantly, a claim about classical charge statistics. Section~7.2 develops this bridge; here we note the scope limit on the surface: Proposition~1 is a statement about \emph{pure} states. For mixed states the $SU(3)$ and $U(1)$ algebras can acquire nonzero quantum pieces (the $U(3)$ entropy stays classical for all states because the algebra is abelian); and the first system in this program where a nonzero quantum piece could appear for pure states needs blocks with both $n_k, m_k > 1$ --- e.g.\ regional subalgebras, or the two-ideal extension of Section~7.3.

\textbf{Corollary 2 ($S_{U(1)} = S_{U(3)}$ for every pure state).} \emph{The $U(1)$ and $U(3)$ commutants have the same center --- the four weight projectors. By Proposition~1, both entropies equal $H(p_0, p_1, p_2, p_3)$.} \hfill$\square$

This holds even though the $U(1)$-invariant algebra is five times larger: gauging the extra $SU(3)$ changes \emph{which states are distinguishable} (the $U(1)$ data distinguishes W from $|001\rangle$; the $U(3)$ data does not) but not the entropy. Entropy is a very lossy summary of $\EA(\rho)$ --- a deflation worth keeping in view whenever a single number is asked to carry structural weight.

\subsection{The Table}

All entropies in bits; sector probabilities exact rationals (machine-checked against the numerics to $10^{-10}$). The rightmost column shows the gauge-\emph{variant} single-qubit entropies for contrast.
...
{\sloppy \emph{Cross-paper note (weighted-W row).} The representative matching state quoted here, $S = (0.9995, 0.8580, 0.7802)$, is the first stored point of the exp2 matching curve (\texttt{weighted\_W.examples[0]} in \texttt{exp2\_results.json}, curve grid index 0). The companion paper's Fig.~1c quotes a different representative, $S = (0.978, 0.895, 0.849)$ --- the separately stored curve midpoint (\texttt{weighted\_W\_example}, grid index 21 of 43). These are different points on the same one-parameter $r_S = 1.8174$ matching curve, not a discrepancy: both satisfy $r_S = \rSM$ to solver precision, both are verified weight-1-supported, and both are pure gauge, with $\SA = 0$ for all three invariant algebras (machine-verified in \texttt{direction\_A/strengthen.py}).\par}

Exact closed forms for the rotor vacuum (machine-checked to $10^{-12}$):
\begin{align*}
...
\textbf{The rotor vacuum} --- the unique ground state of the companion paper's algebra-canonical Hamiltonian $H = i(L_u + 2R_u)$ (\cite{companion}, Sec.~8) --- is the only state in the list with nontrivial invariant data, and that data is two numbers (Section~6.3). Its invariant entropy, 1.724 bits, is comfortably \emph{generic}: the Haar mean is $1.580 \pm 0.239$. Gauge-invariantly, the canonical vacuum is not even unusually entangled.

\subsection{The Headline: the Coupling-Matching Curve Collapses to Pure Gauge}

The companion paper's only surviving static ``match'' was the weighted-W family: a continuous curve of states $a|001\rangle + b|010\rangle + c|100\rangle$ reproducing the Standard Model log-inverse-coupling gap ratio $\rSM = 1.8174$ exactly under the two-parameter logarithmic map (\cite{companion}, Eq.~1; Secs.~5.3, 6.2, Fig.~1c). Every state on that curve is a weight-1 state. Its invariant data is therefore $p = (0, 1, 0, 0)$, identical to that of the Fock state $|001\rangle$, and
\begin{equation*}
\SA = 0 \text{ on the entire curve, for all three algebras}
\end{equation*}
(verified on all five stored curve points; $\max |\SA| = 1.9 \times 10^{-15}$ --- and exact by the support argument, since the conclusion depends only on the curve lying inside the weight-1 sector). The entire matching curve is one $U(3)$ gauge orbit point: gauge-equivalent to a single $\bar d$ quark in a rotated color frame. The gauge equivalence, too, holds for the whole curve by a one-line argument, not by sampling: $SU(3)$ acts transitively on the unit sphere of $\CC^3$, so any unit weight-1 amplitude vector $(a, b, c)$ is the third column of some $U \in SU(3)$, and the corresponding state is $G(U)|001\rangle$ exactly --- the five stored points are numerical confirmation, not the proof. \emph{The static coupling-matching analysis of \cite{companion}, Secs.~5--6, was matching gauge artifacts.} The companion paper diagnosed its title question as ill-posed; the well-posed functional goes further and evaluates the would-be evidence at exactly zero.
...
\begin{quote}
\textbf{Answer to the re-posed question: no --- and now provably, not just diagnostically.} The gauge-invariant entanglement data of a single pure state in this toy is its electric-charge statistics. Charge statistics has no natural three-fold structure aligned with the three gauge factors; the canonical vacuum's statistics are two-parameter degenerate; and $SU(2)_L$ is absent from the construction. The coupling hierarchy is not, and cannot be, encoded in the gauge-invariant entanglement of a single pure state of this toy.
\end{quote}
...
\begin{enumerate}
\item \textbf{Frame-dependent} (\cite{companion}, Secs.~5--6): the static matching analysis succeeded too easily --- matching was statistically generic, null controls matched equally well, and the one algebraically flavored success (the weighted-W curve) survived.
\item \textbf{Ill-posed} (\cite{companion}, Sec.~9.3): the quantity being matched, single-qubit entropy of the Jordan--Wigner factorization, is not gauge-invariant. Diagnosis, not yet answer.
\item \textbf{Well-posed} (this paper, Secs.~3--4): replace subsystem entropy by entanglement relative to the gauge-invariant algebra. The question now has a definite, frame-independent content: three real parameters under $U(3)$.
...
\section{Machine-Checked Lemmas}

Three statements used in the main text are recorded here as machine-checked lemmas rather than numerical observations or structural arguments. Code: \texttt{direction\_A/strengthen.py} (seed 20260609; NumPy + SciPy only); raw outputs in \texttt{direction\_A/strengthen\_results.json}; every claim below is asserted at runtime, so the script fails if any verification breaks.

\subsection{The Mode-Preserving Unitary Group Is \texorpdfstring{$\{e^{i\varphi} G(U) : U \in U(3)\}$}{\{exp(i phi) G(U) : U in U(3)\}}}

\textbf{Lemma B.1.} \emph{Let $F = \mathrm{span}_\CC\{f_1, f_2, f_3\} \subset M_8(\CC)$ be the span of the three Jordan--Wigner modes. Then:}
...
\bibitem{ym2companion} W.T. Trevena, ``Reconstructing the Gauge Coupling from Gauge-Invariant Entanglement Statistics: The Exactly Solvable Case of Two-Dimensional Yang--Mills,'' companion paper (2026); code and manuscript at the same repository.
